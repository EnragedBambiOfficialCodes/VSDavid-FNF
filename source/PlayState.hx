...
class PlayState extends MusicBeatState
{
	...
	private var strumLineNotes:FlxTypedGroup<StaticStrum>;
	public var playerStrums:FlxTypedGroup<StaticStrum>;
	public var dadStrums:FlxTypedGroup<StaticStrum>;
	...
	override public function create()
	{
		...
		strumLineNotes = new FlxTypedGroup<StaticStrum>();
		add(strumLineNotes);

		playerStrums = new FlxTypedGroup<StaticStrum>();
		dadStrums = new FlxTypedGroup<StaticStrum>();
	...
	private function generateStaticArrows(player:Int):Void
	{
		...
		{
			// FlxG.log.add(i);
			var babyArrow:StaticStrum = new StaticStrum(50, strumLine.y);
			...
			switch(player){// this code was made entirely by me
				case 0:
	...
	override public function update(elapsed:Float)
	{
		...
		if(SONG.song.toLowerCase() == 'tutorial'){
			...
			dadStrums.forEachAlive(function(u:FlxSprite){
				u.x += Math.sin(elapsedTime) * 1.3;
				u.angle = 60*Math.cos(((elapsedTime*2) + u.ID*2));
				u.y = strumLine.y+(20*Math.sin((elapsedTime*2) + u.ID*2));
			});
		...
		iconP1.setGraphicSize(Std.int(FlxMath.lerp(150, iconP1.width, 0.8)), Std.int(FlxMath.lerp(150, iconP1.height, 0.8)));// the bruh ever
	...
	override function beatHit()
	{
		...
		if(curBeat % gfSpeed == 0){
			...
			// wish the icon bounce could be like bambis hellscape/bambisona mayhem(fuckkkkkkk)
		}

		if(curBeat % gfSpeed == 4){
			(curBeat % gfSpeed*2) == 4 ? {
				FlxTween.angle(iconP1, -30, 0, Conductor.crochet / 1300 * gfSpeed, {ease: FlxEase.quadOut});
				FlxTween.angle(iconP2, 30, 0, Conductor.crochet / 1300 * gfSpeed, {ease: FlxEase.quadOut});
			}:{
